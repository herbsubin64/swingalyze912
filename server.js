/**
 * Swingalyze Coach v3.0.1
 * Adds manual horizon override from UI to analyzer (optional).
 */
...
function serveFile(res, filePath, contentType='text/html'){
  fs.readFile(filePath,(err,data)=>{ if(err){res.writeHead(404);res.end('Not found');} else {res.writeHead(200,{'Content-Type':contentType});res.end(data);} });
}
...
function runPythonAnalyzer(videoPath, opts={}, timeoutMs=25000){
  ...
  return new Promise((resolve,reject)=>{
    ...
    let out='', err=''; const t=setTimeout(()=>{ child.kill('SIGKILL'); reject(new Error('python analyzer timeout')); }, timeoutMs);
    child.stdout.on('data',d=>out+=d.toString());
    child.stderr.on('data',d=>err+=d.toString());
    child.on('close',code=>{ clearTimeout(t); if(code===0){ try{ resolve(JSON.parse(out)); }catch{ reject(new Error('python json parse fail')); } } else { reject(new Error(err||`python exit ${code}`)); } });
  });
}
...
const server = http.createServer((req,res)=>{
  ...
  if (req.method==='POST' && pathname==='/api/upload') {
    ...
    if (!ct.startsWith('video/') && ct!=='application/octet-stream') return sendJSON(res,400,{ ok:false, error:'Send raw video bytes (Content-Type video/*)' });
    const id=uid(); const filePath = path.join(UPLOAD_DIR, `${id}.mp4`);
    const ws=fs.createWriteStream(filePath); let written=0;
    req.on('data',chunk=>{ written+=chunk.length; if (written>MAX_UPLOAD_BYTES){ ws.destroy(); fs.unlink(filePath,()=>{}); req.destroy(); }});
    req.pipe(ws);
    ws.on('finish',()=>{ if(written>MAX_UPLOAD_BYTES) return sendJSON(res,413,{ ok:false, error:`File too large (> ${Math.round(MAX_UPLOAD_BYTES/1024/1024)}MB)` });
      jobs.set(id,{ status:'queued', path:filePath, created:Date.now() }); return sendJSON(res,200,{ ok:true, jobId:id, path:filePath }); });
    ws.on('error',e=>sendJSON(res,500,{ ok:false, error:e.message })); return;
  }

  if (req.method==='GET' && pathname.startsWith('/api/job/')) {
    const id=pathname.split('/').pop(); const job=jobs.get(id);
...
server.listen(PORT,'0.0.0.0',()=>{ console.log(`[srv] Listening on ${PORT}`); console.log(`[srv] Open: http://localhost:${PORT}`); });
