(function(){
  ...
  const exportPngBtn=document.getElementById('exportPng'); const exportJsonBtn=document.getElementById('exportJson'); const exportPdfBtn=document.getElementById('exportPdf');
  ...
  // Playback (simple)
  function stepFrames(n){ const fps=30; player.currentTime=Math.max(0, player.currentTime + n*(1/fps)); }
  document.querySelectorAll('[data-step]').forEach(btn=>btn.addEventListener('click',()=>{ stepFrames(Number(btn.getAttribute('data-step'))||0); }));

  // Drawing handlers
  let drawing=null;
  overlay.addEventListener('mousedown',e=>{
    if(drawMode.value==='none') return;
    const r=overlay.getBoundingClientRect(); const x=(e.clientX-r.left)*((player.videoWidth||overlay.width)/overlay.clientWidth); const y=(e.clientY-r.top)*((player.videoHeight||overlay.height)/overlay.clientHeight);
  ...
  document.getElementById('setFrame').addEventListener('click',()=>{
    const w=320,h=180; const c=document.createElement('canvas'); c.width=w; c.height=h; const cx=c.getContext('2d'); try{ cx.drawImage(player,0,0,w,h);}catch{} state[currentSlot].frameSec=player.currentTime||0; state[currentSlot].thumb=c.toDataURL('image/png');
  });

  // Upload -> /api/upload (no autoplay)
  videoInput.addEventListener('change', async (e)=>{
    const file=e.target.files[0]; if(!file) return;
    const url=URL.createObjectURL(file); player.src=url; player.pause(); player.currentTime=0;
  ...
  // Analyzer horizon override: use Impact->Ground line if present
  ...
  analyzeBtn.addEventListener('click', async ()=>{
    ...
    const hz = impactHorizonDegOrNull(); if (hz !== null && isFinite(hz)) body.horizon_deg = hz;
    ...
    const pc=j.pose?.confidence||0; const tempoC=j.tempo?.confidence||0;
    ...
    tipsEl.innerHTML=''; const tips=[];
    if (tempoC<0.4) tips.push("Tempo provisional — camera/light may reduce accuracy.");
  ...
  exportJsonBtn.addEventListener('click', ()=>{
    const payload={ build:'Swingalyze Coach v3.1.0', generated_at:new Date().toISOString(), analyzer:lastAnalyze||null, upload_job_id: window.currentJobId||null, slots:{} };
  ...
  // Export PNG (same strip as before, compact)
  exportPngBtn.addEventListener('click', ()=>{
    ...
    cx.fillText('Swingalyze Coach v3.1.0 — 5-keyframe report',10,h+24);
  ...
  // Export PDF (optionally upload to S3)
  exportPdfBtn.addEventListener('click', async ()=>{
    const payload={ meta:{ title:'Swingalyze Coach Report', notes:'Angles and tempo at impact.', build:'Swingalyze Coach v3.1.0' }, report:lastAnalyze||{}, slots:{}, s3: !!s3toggle?.checked };
    for(const s of slots){ payload.slots[s]={ thumbnail_png: state[s].thumb, measured_angles: state[s].angles, frame_sec: state[s].frameSec }; }

    const r = await fetch('/api/export/pdf',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });

    if (payload.s3) {
      // expecting JSON with signed URL
      const j = await r.json().catch(()=>null);
      if (j?.ok && j?.url) {
        const a=document.createElement('a'); a.href=j.url; a.target='_blank'; a.rel='noopener'; a.click();
      } else {
        alert('S3 upload failed — check server logs and env.');
      }
    } else {
      // direct PDF download
      const blob = await r.blob();
      const url = URL.createObjectURL(blob);
      const a=document.createElement('a'); a.href=url; a.download='swingalyze_report.pdf'; a.click(); URL.revokeObjectURL(url);
    }
  });
